...
class TimingLogger:
    def __init__(self, max_iters=1000):
        ...
        self.loop_times = deque(maxlen=self.max_iters-1)
    
    def next(self):
        """ Start the timer. """
        self.base_times.append(time.time())
        if len(self.base_times) > 1:
            self.loop_times.append(self.base_times[-1] - self.base_times[-2])
    ...
    @property
    def fps(self, last_n=100):
        """ Get the FPS based on the last iteration. """
        if len(self.loop_times) < last_n:
            last_n = len(self.loop_times)
            if last_n == 0:
                return 0.0
        return 1/np.mean(list(self.loop_times)[-last_n:])
    ...
    def report(self, include_iters=True, time_unit='s', align='center', precision=None, ignore_init_iters=0):
        ...
        print("\nSummary Statistics:")
        print(f"Mean FPS: {1/np.mean(self.loop_times):.1f}\n")
